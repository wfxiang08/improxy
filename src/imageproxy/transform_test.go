...
// go test imageproxy -v -run "TestResizeParams01"
func TestResizeParams01(t *testing.T) {
	src := image.NewNRGBA(image.Rect(0, 0, 64, 128))
	w, h, resize := resizeParams(src, Options{Width: 100, Height: 100})

	// w, h, resize
	// 输出应该是64*64
	fmt.Printf("W: %d, H: %d, Resize: %t\n", w, h, resize)
}
func TestResizeParams(t *testing.T) {
	...
	tests := []struct {
		...
		{Options{Width: 100, Height: 200}, 100, 200, true},
...
func TestTransform(t *testing.T) {
	...
	for _, tt := range tests {
		...
		out, _, err := Transform(in, emptyOptions)
		if err != nil {
		...
		out, _, err = Transform(in, Options{Width: -1, Height: -1})
		if err != nil {
			t.Errorf("Transform with encoder %s returned unexpected error: %v", tt.name, err)
		}
		if len(out) == 0 {
	...
	if _, _, err := Transform([]byte{}, Options{Width: 1}); err == nil {
