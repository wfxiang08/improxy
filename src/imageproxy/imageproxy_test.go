...
// go test imageproxy -v -run "TestAllowed"
func TestAllowed(t *testing.T) {
	...
	tests := []struct {
		...
		// 没有任何限制，直接放行
		{"http://test/image/0", nil, nil, nil, true},

		// 验证 whitelist
		{"http://good/image/1", whitelist, nil, nil, true},
		{"http://bad/image/2", whitelist, nil, nil, false},

		// 验证 referrer
		{"http://test/image/3", nil, whitelist, genRequest(map[string]string{"Referer": "http://good/foo"}), true},
		{"http://test/image/4", nil, whitelist, genRequest(map[string]string{"Referer": "http://bad/foo"}), false},
		{"http://test/image/5", nil, whitelist, genRequest(map[string]string{"Referer": "MALFORMED!!"}), false},
		{"http://test/image/6", nil, whitelist, genRequest(map[string]string{}), false},

		// 验证签名
		{fmt.Sprintf("http://good/%s", media_utils.SimpleSignUrl("image/7", "", 3600*24*7)), nil, nil, nil, true},
	}
	...
	for _, tt := range tests {
		...
		req := &Request{u, emptyOptions, tt.request}
		if got, _ := p.allowed(req); (got == nil) != tt.allowed {
			t.Errorf("allowed(%q) returned %v, want %v.\nTest struct: %#v", req, got, tt.allowed, tt)
		}
